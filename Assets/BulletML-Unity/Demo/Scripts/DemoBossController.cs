...
namespace Pixelnest.BulletML.Demo
{
  public class DemoBossController : MonoBehaviour
  {
    ...
    private Animator bossAnimationStatus;

    void Start()
    {
        bossAnimationStatus = this.GetComponent<Animator>();
      NewMoveTarget();
    ...
    void Update()
    {
      ...
      if (direction.x > 0)
      {
          bossAnimationStatus.SetBool("Right", true);
          bossAnimationStatus.SetBool("Left", false);
      }
      else if (direction.x < 0)
      {
          bossAnimationStatus.SetBool("Left", true);
          bossAnimationStatus.SetBool("Right", false);
      }
      else
      {
          bossAnimationStatus.SetBool("Right", false);
